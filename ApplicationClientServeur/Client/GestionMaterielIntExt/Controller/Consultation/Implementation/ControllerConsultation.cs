...
internal class ControllerConsultation : IControllerConsultation
{
    ...
    public async Task<bool> OpenViewConsultation()
    {
        ...
        {
            TrConsultationResponse trResponse;
            //var responseConsultationDto = await _mconsult.GetConsultationMateriels();
            try
            {
                trResponse = await _mconsult.GetConsultationMateriels();

            }
            catch (SysException e)
            {
                //var detail = JsonConvert.DeserializeObject<EProblemDetails>(e.Data);
    ...
    public async Task<IMViewConsultation> GetMaterielsInfos(Categorie categorie)
    {
        ...
        {
            TrConsultationResponse responseTransian;
            try
            {
                responseTransian = await _mconsult.GetConsultationMateriels(categorie.Id);
            }
            catch (SysException e)
            {
                _messageBoxService.InformationErrorUserOK(e.Message);
                var myIMViewConsultationError = _mviewConsult.FactoMViewConsultation();
                myIMViewConsultationError.Exp = e;
            ...
            catch (Exception e)
            {
                _messageBoxService.InformationErrorUserOK("Erreur système");
                var myIMViewConsultationError = _mviewConsult.FactoMViewConsultation();
                myIMViewConsultationError.Exp = new SysException("Erreur système");
                return myIMViewConsultationError;
            }
            ...
            {
                var myIMViewConsultationError = _mviewConsult.FactoMViewConsultation();
                myIMViewConsultationError.Exp = new SysException("Erreur système de réponse");
                return myIMViewConsultationError;
            }
        }
        else
    ...
    public async Task<IMViewConsultation> GetMaterielsInfos()
    {
        ...
        {
            //var responseConsultationDto = await _mconsult.GetConsultationMateriels();
            trResponse = await _mconsult.GetConsultationMateriels(); // TODO ici
        }
        catch (SysException e)
        {
            _messageBoxService.InformationErrorUserOK(e.Message);
